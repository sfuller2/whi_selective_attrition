##################################################
## Create super learner for predicting censoring (colorectal/ltfu)
## Project: WHI
...
cr <- data %>% filter(cancer == "Colorectal")
...
for (i in 0:16) {
  ...
  #denom_preds <- long_sl_denom(dat, i, "ltfu", "cr")

  # calculate the numerators
  num_preds <- long_sl_num(dat, i, "ltfu", "cr")

  today <- str_remove_all(Sys.Date(), "-")

  #denom_file <- paste0("output/weights/cr_ltfu/", today, "_denom_wv_", paste0(i, ".rds"))
  num_file <- paste0("output/weights/cr_ltfu/", today, "_num_wv_", paste0(i, ".rds"))
  ...
  #filename1 <- paste0("output/r_objects/cr_ltfu/", today, "_sl_fit_denom_wv_", paste0(i, ".rds"))
  filename2 <- paste0("output/r_objects/cr_ltfu/", today, "_sl_fit_num_wv_", paste0(i, ".rds"))
